...
if ( ! function_exists( 'am750_entry_footer' ) ) :
	...
	function am750_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma  // EX ETIQUETAS*/
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'am750' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Temas: %1$s', 'am750' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
